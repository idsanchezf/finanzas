...
class ITransaccionRepository(ABC):
    ...
    @abstractmethod
    async def get_by_usuario(
        self, usuario_id: UUID, filters: dict[str, Any] | None = None, page: int = 1, size: int = 50
    ) -> tuple[list[Any], int]: ...
...
class ICategoriaRepository(ABC):
    ...
    @abstractmethod
    async def get_predefinidas(self) -> list[Any]: ...
    ...
    @abstractmethod
    async def buscar_por_nombre(self, nombre: str, usuario_id: UUID | None = None) -> list[Any]:
