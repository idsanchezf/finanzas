...
class TestGetByUsuario:
    ...
    async def test_Should_FilterBySearch_When_SearchProvided(
        ...
        t1 = TransaccionModel(
            ...
            fecha=date(2026, 6, 1), comercio_original="NETFLIX STREAMING",
            valor=Decimal("15"),
        )
        t2 = TransaccionModel(
            id=uuid.uuid4(), extracto_id=extracto_id, usuario_id=usuario_id,
            fecha=date(2026, 6, 2), comercio_original="SPOTIFY PREMIUM",
            valor=Decimal("10"),
        )
        ...
        # Act ------------------------------------------------------------
        items, total = await repo.get_by_usuario(
            usuario_id, filters={"search": "netflix"}
        )
